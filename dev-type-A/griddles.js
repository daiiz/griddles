...
griddles.load = function() {
    ...
    if (ww != griddles.pre_width || griddles.render == true) {
        ...
        if (max_stream_nums > 0) {
            ...
            var lr = (ww - scrollbar_width) -sw; //w85pc - sw; 
            if (lr > 0) {
                lf = lr / 2;
            } else {
                lf = 0;
...
griddles.renderTime = function(y, lg, cards, n) {
     ...
     //d.getElementById("stream_" + intMinStream).innerHTML = d.getElementById("stream_" + intMinStream).innerHTML + content;
     window.setTimeout(function() {
        //streamHeights = griddles.setStreamHeights(n);
        //console.log(document.getElementById("stream_"+intMinStream).offsetHeight);
        if(y+1 < lg) {
...
griddles.createContent = function(cards, y) {
             ...
             content = '<div class="Card" ' + card_id + 'style="display:block; ' + v + 'margin-bottom:' + b + 'px; width:' + w + 'px;'  + tit + '">' + 
